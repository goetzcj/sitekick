...
- Use `GOOGLECALENDAR_GET_CURRENT_DATE_TIME` to anchor local time. Prefer the timezone from connection/current-user calendar info.
- Use `GOOGLECALENDAR_EVENTS_LIST_ALL_CALENDARS` with bounded RFC3339 `time_min` / `time_max`, `single_events=true`, `show_deleted=false`.
- Start with `response_detail="minimal"`; hydrate/get full details only for ambiguous prep needs.
- Check `summary_view` and `events`; minimal responses may put usable rows in `summary_view`.
