...
- `references/composio-mcp.md` — Composio MCP setup from composio.dev/hermes, OAuth/PKCE callback handling in WSL/headless environments, app-toolkit connection workflow, and Google app triage patterns for Gmail, Calendar, and Tasks.
- `references/importing-third-party-skill-repos.md` — Installing multi-skill GitHub repos, handling tap/search gaps, scanner CAUTION overrides, linked reference files, and verification.
...
When importing a GitHub repository that contains multiple skills, inspect the repo for every `*/SKILL.md` and install each one if needed. Adding a tap or searching may expose only a subset. Direct `SKILL.md` installs may omit linked `references/`, `templates/`, or `scripts/`; verify and copy support files when present. See `references/importing-third-party-skill-repos.md`.
...
#### Composio hosted MCP quick setup

When the user asks to set up Composio from `composio.dev/hermes`, add an HTTP MCP server named `composio` at `https://connect.composio.dev/mcp` with OAuth auth and no manual auth headers:
...
### Installing a GitHub repo of skills with linked files
When importing a community repo that contains multiple skill directories, each with `SKILL.md` plus `references/`, direct `hermes skills install https://.../SKILL.md` installs the main `SKILL.md` but may not bring along linked reference files. Reliable pattern:
1. Add the repo as a tap when useful: `hermes skills tap add owner/repo`
2. Install each skill by raw `SKILL.md` URL, using `--category <category> --yes`; use `--force` only after reviewing scan warnings and when the user explicitly trusts the source.
3. Clone the repo to a temp directory and copy each skill's `references/`, `templates/`, or `scripts/` directories into the installed skill directory under the active profile.
4. Verify with `hermes skills list` and one `skill_view` call against a skill that has linked files.
