...
# After AGENTMAIL_API_KEY is present:
set -a
. /path/to/profile/.env
set +a
curl -sS -w '\nHTTP_STATUS:%{http_code}\n' \
  -H "Authorization: Bearer ${AGENTMAIL_API_KEY}" \
  'https://api.agentmail.to/v0/inboxes?limit=10'
```

A successful test returns HTTP 200 with an `inboxes` array. To verify message access, URL-encode the inbox address:

```bash
curl -sS -w '\nHTTP_STATUS:%{http_code}\n' \
  -H "Authorization: Bearer ${AGENTMAIL_API_KEY}" \
  'https://api.agentmail.to/v0/inboxes/sitekick%40agentmail.to/messages?limit=5'
```

Do not send messages or drafts during setup unless the user explicitly approves that side effect.
